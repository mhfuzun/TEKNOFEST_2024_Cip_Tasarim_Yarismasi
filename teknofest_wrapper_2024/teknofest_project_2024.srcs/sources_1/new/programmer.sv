...
// Düzenlemeler: 
//  * (23/08/2024) - x
//      - DEFAULT_DIV = (CPU_FREQ_HZ / UART_BAUD_RATE)-1; olarak yeniden ayarlandı
//      - ilk programlamanın ardından yapılacak diğer yüklemer için <prog_addr> kaydı için sıfırlanma zamanı eklendi
...
module programmer #(
  ...
  input  logic         mem_gnt, // kullanılmıyor (DDR'ın yazma için daima hazır olduğu varsayılıyor)
  output logic         mem_we,
  ...
  localparam DEFAULT_DIV = (CPU_FREQ_HZ / UART_BAUD_RATE)-1;
    ...
    case(state_prog)
      ...
      SequenceCheck: begin
        if(received_sequence == PROGRAM_SEQUENCE) begin
          ...
          state_prog_next = SequenceWait;
          $display("[%0t] Programlayıcı: Sequence uyuşmazlığı.", $realtime);
        end
      end
  ...
  always @(posedge clk) begin
    if (!rst_n) begin // eski hali: (!(rst_n && system_reset_no))
      ...
      /*
        daha sonraki yükleme işlemleri için addr sıfırlanır.
      */
      if (state_prog == SequenceLengthCalc) begin
        prog_addr <= 'h0;
      end else if ((state_prog == SequenceProgram) && prog_inst_valid) begin
  ...
  always_ff@(posedge clk) begin
    if(~rst_n) begin
      ...
      SequenceProgram: begin
        system_reset_no       <= 1'b0; // gelen yeni bir istek ile tekrardan programlama durumuna geçilir.
        if (recv_buf_valid) begin
          ...
          if (&instruction_byte_ctr) begin
            ...
            /*
                programlayıcı her 128 bit (4 buyruk) ile yazma yaptığı için bilgisayarda 
                kullanılanılan programlayıcı yazılımı, buyruk listesini 4 katı olacak şekilde boş 
                satırlar ile tamamlamalıdır. Aksi halde sonda bulunan buyruklar belleğe yazılmaz
                ayrıca programlayıcı modülü hata verip bu status'de sabit kalır.
            */
